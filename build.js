  ...
  for (const folder of resourceFoldersToCopy) {
    fs.traverseTreeSync(
      ...
      fileName => {
        ...
        console.log("*** entry",relative, entry);
  ...
  const bundle = ids => {
    ...
    return bundler.bundle((error, content) => {
      if (error != null) {
        ...
        try {
          console.log("pth",root + '/scripts/testpage.html')
          fs.symlinkSync(root + '/scripts/testpage.html', outDir + '/testpage.html');
        } catch(e) {
          console.log('failed to generate simlink')
          // do nothing
        }
